...
mod tests {
    ...
    fn setup() {
        ...
        INIT.get_or_init(|| {
            let tmp = std::env::temp_dir()
                .join(format!("i-rs-tick-test-{}", std::process::id()));
            // ensure dir exists
    ...
    #[test]
    fn test_help() {
        let _ = Cli::try_parse_from(["i-rs-tick", "--help"]);
    }
    ...
    fn test_add_and_list() {
        ...
        let cmd = Commands::Add {
            task_name: "test-i-rs-tick-1".to_string(),
            duration_seconds: 1,
            started_at: None,
            description: None,
            tag: vec![],
        ...
        assert!(run(cmd, crate::presentation::OutputFormat::Table).is_ok());
        let cmd = Cli::try_parse_from(["i-rs-tick", "list"]).unwrap().command;
        assert!(run(cmd, crate::presentation::OutputFormat::Table).is_ok());
    }
