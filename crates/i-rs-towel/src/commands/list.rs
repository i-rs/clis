use crate::models::{TowelRow, ListItem};
...
pub fn handle_list(tag: Option<String>, format: OutputFormat) -> Result<()> {
    ...
    let entries: Vec<&crate::models::TowelEntry> = storage::filter_by_tag(&store, tag.as_deref());
    ...
    let rows: Vec<TowelRow> = entries.iter().map(|e| TowelRow::from_entry(e)).collect();
