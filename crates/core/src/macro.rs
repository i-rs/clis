...
/// Generate the complete CLI entrypoint (`main.rs`) for a standard i-rs tool.
///
/// Replaces the repetitive `struct Cli` + `enum Commands` + `fn main()` + `fn run()`
/// boilerplate (~120 lines per tool) with a single macro invocation.
///
/// ## What it generates
///
/// - `struct Cli` (clap Parser)
/// - `enum Commands` (clap Subcommand with standard CRUD variants)
/// - `fn main()` (parse + exit_on_error)
/// - `fn run()` (match dispatch to hand-written handlers)
///
/// ## Usage in `crates/i-rs-xxx/src/main.rs`:
///
/// ```ignore
/// mod commands;
/// mod models;
...
///     add_args: { weight: f64 => "体重值(kg)", date: Option<String> => "日期(YYYY-MM-DD)" },
///     update_args: { weight: Option<f64> => "体重值(kg)" },
///     list_args: { days: Option<usize> => "最近N天", chart: bool => "图表", stats: bool => "统计" },
/// }
/// ```
///
/// The generated `run()` dispatches to:
/// - `commands::handle_add(...)` — receives parsed Add args + `&OutputFormat`
/// - `commands::handle_delete(...)` — receives id + `&OutputFormat`
/// - `commands::handle_list(...)` — receives parsed List args + `&OutputFormat`
/// - `commands::handle_update(...)` — receives id + parsed Update args + `&OutputFormat`
/// - `commands::handle_get(...)` — receives id + `&OutputFormat`
/// - `commands::handle_example()`
/// - `commands::handle_skill(&SkillCommand)`
/// - `commands::data::handle(&DataCommand)`
///
/// Default fields always added to every Add/Update:
/// - `#[arg(short, long)] tag: Vec<String>` / `Option<Vec<String>>`
    ...
    (
        ...
        enum Commands {
            ...
            List {
                ...
                #[arg(short = 'L', long, help = "Max entries to return")]
                limit: Option<usize>,
        ...
        fn main() {
            ...
            $crate::exit_on_error!(run(cli.command, format), cli.json);
        }

        fn run(command: Commands, format: OutputFormat) -> ::anyhow::Result<()> {
            match command {
                Commands::Add { $($($add_name,)*)? tag, remark } => {
                    handle_add($($($add_name,)*)? tag, remark, format)?;
                }
                Commands::Delete { id } => {
                ...
                Commands::List { $($($list_name,)*)? limit, offset } => {
                    handle_list($($($list_name,)*)? limit, offset, format)?;
                }
                Commands::Update { id, $($($upd_name,)*)? tag, remark } => {
                    handle_update(id, $($($upd_name,)*)? tag, remark, format)?;
                }
                Commands::Example {} => {
                    handle_example();
                }
