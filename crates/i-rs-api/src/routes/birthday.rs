...
async fn update_birthday(
    ...
    let entry = state.birthday.write(|store| -> Result<_, ApiError> {
        let entry = store.birthdays.get_mut(&id)
...
pub fn router() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/{id}", delete(delete_birthday).put(update_birthday))
}
...
async fn get_birthday(
    ...
    let entry = state.birthday.read(|store| {
        store.birthdays.get(&id).cloned().ok_or_else(|| ApiError::NotFound(format!("Birthday '{id}' not found")))
    })?;
    Ok(ok_json(entry))
}
