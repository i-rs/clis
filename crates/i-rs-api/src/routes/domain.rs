...
async fn update_domain(
    ...
    let entry = state.domain.write(|store| -> Result<_, ApiError> {
        let entry = store
            .domains
            .get_mut(&id)
            .ok_or_else(|| ApiError::NotFound(format!("Domain '{id}' not found")))?;
...
pub fn router() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/{id}", delete(delete_domain).patch(update_domain))
}
...
async fn list_domains(State(state): State<Arc<AppState>>) -> ApiResult<Json<serde_json::Value>> {
