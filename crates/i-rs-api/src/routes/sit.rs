...
pub fn router() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/{id}", delete(delete_sit).patch(update_sit))
}
...
async fn list_sits(State(state): State<Arc<AppState>>) -> ApiResult<Json<serde_json::Value>> {
...
async fn add_sit(
    ...
    let entry =
        i_rs_sit::models::SitEntry::new(duration_minutes, started_at, ended_at, tags, remark);
