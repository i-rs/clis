...
pub fn format_debt_detail(debt: &Debt) -> String {
    ...
    lines.push(format!("{}: {:.2}", "Total Amount".cyan().bold(), detail.total_amount));
    lines.push(format!("{}: {:.2}", "Paid Amount".cyan().bold(), detail.paid_amount));
    lines.push(format!("{}: {:.2}", "Remaining".cyan().bold(), detail.remaining));
...
pub fn format_stats(stats: &Stats) -> String {
    ...
    lines.push(format!("{}: {}", "Total Debts".cyan().bold(), stats.total_debts));
    lines.push(format!("{}: {:.2}", "Total Amount".cyan().bold(), stats.total_amount));
    lines.push(format!("{}: {:.2}", "Total Paid".cyan().bold(), stats.total_paid));
    lines.push(format!("{}: {:.2}", "Total Remaining".cyan().bold(), stats.total_remaining));
    lines.push(format!("{}: {}", "Overdue Count".cyan().bold(), stats.overdue_count));
    if !stats.by_type.is_empty() {
        lines.push(String::new());
        lines.push("=== By Type ===".cyan().bold().to_string());
        for (type_name, type_stats) in &stats.by_type {
            lines.push(String::new());
            lines.push(format!("{} ({})", type_name, type_stats.count).cyan().bold().to_string());
