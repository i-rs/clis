use crate::models::{CycleRow, ListItem};
...
pub fn handle_list(tag: Option<String>, format: OutputFormat) -> Result<()> {
    ...
    let entries: Vec<&crate::models::CycleEntry> = storage::filter_by_tag(&store, tag.as_deref());
    ...
    let rows: Vec<CycleRow> = entries.iter().map(|e| CycleRow::from_entry(e)).collect();
