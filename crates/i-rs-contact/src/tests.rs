...
mod tests {
    ...
    fn setup() {
        ...
        INIT.get_or_init(|| {
            let tmp = std::env::temp_dir()
                .join(format!("i-rs-contact-test-{}", std::process::id()));
            // ensure dir exists
    ...
    #[test]
    fn test_help() {
        let _ = Cli::try_parse_from(["i-rs-contact", "--help"]);
    }
    ...
    fn test_crud() {
        ...
        let cmd = Commands::Add {
            name: "test-i-rs-contact-1".to_string(),
            phone: None,
            email: None,
            relationship: None,
            tag: vec![],
        ...
        assert!(run(cmd, crate::presentation::OutputFormat::Table).is_ok());
        let cmd = Cli::try_parse_from(["i-rs-contact", "list"]).unwrap().command;
        assert!(run(cmd, crate::presentation::OutputFormat::Table).is_ok());
        let cmd = Commands::Add {
            name: "test-i-rs-contact-1b".to_string(),
            phone: None,
            email: None,
            relationship: None,
            tag: vec![],
