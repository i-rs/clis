use crate::models::{PurifyRow, ListItem};
...
pub fn handle_list(tag: Option<String>, format: OutputFormat) -> Result<()> {
    ...
    let entries: Vec<&crate::models::PurifyEntry> = storage::filter_by_tag(&store, tag.as_deref());
    ...
    let rows: Vec<PurifyRow> = entries.iter().map(|e| PurifyRow::from_entry(e)).collect();
