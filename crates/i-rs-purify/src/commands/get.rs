...
pub fn handle_get(id: String, format: OutputFormat) -> Result<()> {
    ...
    print_header(&format!("Purify: {}", display_id.green()));
    println!();

    let style = OwoStyle::new().bold();

    println!("{:16} {}", "Filter type:".style(style), entry.filter_type.cyan());
    println!("{:16} {}", "Replaced at:".style(style), entry.replaced_at.format("%Y-%m-%d %H:%M").to_string().yellow());
