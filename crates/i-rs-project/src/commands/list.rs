...
use crate::presentation::{format_project_table, output_list, print_project_count, print_warning, OutputFormat};
use crate::storage;
use anyhow::Result;
...
pub fn handle_list(tag: Option<String>, status: Option<String>, format: OutputFormat) -> Result<()> {
    ...
    let projects: Vec<&crate::models::Project> = store.projects.iter().filter(|p| {
        let tag_match = tag.as_ref().map_or(true, |t| p.tags.iter().any(|pt| pt.eq_ignore_ascii_case(t)));
