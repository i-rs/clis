use crate::models::{ToothbrushRow, ListItem};
...
pub fn handle_list(tag: Option<String>, format: OutputFormat) -> Result<()> {
    ...
    let entries: Vec<&crate::models::ToothbrushEntry> = storage::filter_by_tag(&store, tag.as_deref());
    ...
    let rows: Vec<ToothbrushRow> = entries.iter().map(|e| ToothbrushRow::from_entry(e)).collect();
