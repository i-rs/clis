use crate::models::{ListItem, SubRow};
...
pub fn handle_list(tag: Option<String>, format: OutputFormat) -> Result<()> {
    ...
    let entries: Vec<&crate::models::SubEntry> = storage::filter_by_tag(&store, tag.as_deref());

    i_rs_core::handle_empty!(entries, format, tag.as_deref(), "No subscriptions found.");
    ...
    let rows: Vec<SubRow> = entries.iter().map(|e| SubRow::from_entry(e)).collect();
