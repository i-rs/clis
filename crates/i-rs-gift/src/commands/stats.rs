...
pub fn handle_stats() -> Result<()> {
    ...
    let sent_count = gifts
        .iter()
        .filter(|g| g.gift_type == GiftType::Sent)
        .count();
    let received_count = gifts
        .iter()
        .filter(|g| g.gift_type == GiftType::Received)
        .count();

    let sent_value: f64 = gifts
        .iter()
        .filter(|g| g.gift_type == GiftType::Sent)
        .map(|g| g.value)
        .sum();
    let received_value: f64 = gifts
        .iter()
        .filter(|g| g.gift_type == GiftType::Received)
        .map(|g| g.value)
        .sum();

    let mut occasion_counts: std::collections::BTreeMap<String, usize> =
        std::collections::BTreeMap::new();
    for gift in &gifts {
        *occasion_counts.entry(gift.occasion.clone()).or_insert(0) += 1;
    }

    let mut recipient_counts: std::collections::BTreeMap<String, usize> =
        std::collections::BTreeMap::new();
    for gift in &gifts {
    ...
    if received_count > 0 {
        println!(
            "  {:12} {:.2}",
            "Avg Received:".dimmed(),
            received_value / received_count as f64
        );
    }
