use crate::models::{DoseRow, ListItem};
...
pub fn handle_list(tag: Option<String>, format: OutputFormat) -> Result<()> {
    ...
    let entries: Vec<&crate::models::DoseEntry> = storage::filter_by_tag(&store, tag.as_deref());
    ...
    let rows: Vec<DoseRow> = entries.iter().map(|e| DoseRow::from_entry(e)).collect();
