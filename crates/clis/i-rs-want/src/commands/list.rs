use crate::models::{ListItem, WantRow};
...
pub fn handle_list(tag: Option<String>, format: OutputFormat) -> Result<()> {
    ...
    let entries: Vec<&crate::models::WantEntry> = storage::filter_by_tag(&store, tag.as_deref());

    i_rs_core::handle_empty!(entries, format, tag.as_deref(), "No items found.");
    ...
    let rows: Vec<WantRow> = entries.iter().map(|e| WantRow::from_entry(e)).collect();
