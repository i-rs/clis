use crate::models::{ListItem, SheetRow};
...
pub fn handle_list(tag: Option<String>, format: OutputFormat) -> Result<()> {
    ...
    let entries: Vec<&crate::models::SheetEntry> = storage::filter_by_tag(&store, tag.as_deref());
    ...
    let rows: Vec<SheetRow> = entries.iter().map(|e| SheetRow::from_entry(e)).collect();
