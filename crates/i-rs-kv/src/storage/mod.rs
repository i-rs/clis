...
pub fn load_store() -> anyhow::Result<KvStore> {
    let mut storage = Storage::<KvStore>::new("kv");
    storage.load()?;
    Ok(storage.data)
}

pub fn save_store(store: &KvStore) -> anyhow::Result<()> {
    let storage = Storage::<KvStore>::new("kv");
