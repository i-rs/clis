...
mod tests {
    ...
    fn setup() {
        ...
        INIT.get_or_init(|| {
            let tmp = std::env::temp_dir()
                .join(format!("i-rs-want-test-{}", std::process::id()));
            // ensure dir exists
    ...
    #[test]
    fn test_help() {
        let _ = Cli::try_parse_from(["i-rs-want", "--help"]);
    }
    ...
    fn test_crud() {
        ...
        let cmd = Commands::Add {
            name: "test-i-rs-want-1".to_string(),
            url: None,
            price: None,
            currency: None,
            priority: "test-priority".to_string(),
            tag: vec![],
            remark: vec![],
        };
        assert!(run(cmd, crate::presentation::OutputFormat::Table).is_ok());
        let cmd = Cli::try_parse_from(["i-rs-want", "list"]).unwrap().command;
        assert!(run(cmd, crate::presentation::OutputFormat::Table).is_ok());
        let cmd = Commands::Add {
            name: "test-i-rs-want-1b".to_string(),
            url: None,
            price: None,
            currency: None,
            priority: "test-priority".to_string(),
