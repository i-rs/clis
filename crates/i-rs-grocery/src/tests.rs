...
mod tests {
    ...
    fn setup() {
        ...
        INIT.get_or_init(|| {
            let tmp = std::env::temp_dir()
                .join(format!("i-rs-grocery-test-{}", std::process::id()));
            // ensure dir exists
    ...
    #[test]
    fn test_help() {
        let _ = Cli::try_parse_from(["i-rs-grocery", "--help"]);
    }
    ...
    fn test_crud() {
        ...
        let cmd = Commands::Add {
            name: "test-i-rs-grocery-1".to_string(),
            quantity: 1,
        ...
        assert!(run(cmd, crate::presentation::OutputFormat::Table).is_ok());
        let cmd = Cli::try_parse_from(["i-rs-grocery", "list"]).unwrap().command;
        assert!(run(cmd, crate::presentation::OutputFormat::Table).is_ok());
        let cmd = Commands::Add {
            name: "test-i-rs-grocery-1b".to_string(),
            quantity: 1,
            unit: "test-unit".to_string(),
            tag: vec![],
            remark: vec![],
