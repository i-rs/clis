...
pub fn stats(args: StatsArgs, output_format: OutputFormat) -> anyhow::Result<()> {
    ...
    let completed: usize = goals
        .iter()
        .filter(|g| g.current_amount >= g.target_amount)
        .count();
    let overdue: usize = goals
        .iter()
        .filter(|g| g.days_until_deadline() < 0 && g.current_amount < g.target_amount)
        .count();
    ...
    match output_format {
        OutputFormat::Json => {
            let tag_json: HashMap<String, serde_json::Value> = tag_stats
                .into_iter()
                .map(|(k, v)| {
        ...
        OutputFormat::Table | OutputFormat::Default => {
            ...
            println!(
                ...
                format!("{overall_progress:.1}%").green()
            );
            println!(
                "{}",
                crate::presentation::print_progress_bar(overall_progress, 50)
            );
