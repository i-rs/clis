        ...
        Some(goal) => {
            match output_format {
                ...
                OutputFormat::Table | OutputFormat::Default => {
                    ...
                    println!("\n{} {} of {} milestones reached", "Progress:".dimmed(), reached, goal.milestones.len());
                }
            }
        }
        None => {
...
pub fn add_milestone(args: MilestoneArgs, output_format: OutputFormat) -> anyhow::Result<()> {
    let name = args.name.ok_or_else(|| anyhow::anyhow!("Milestone name is required"))?;
    ...
    match output_format {
        ...
        OutputFormat::Table | OutputFormat::Default => {
            ...
            let reached = updated_goal.milestones.iter().any(|m| m.name == name && m.reached);
            if reached {
...
pub fn remove_milestone(args: MilestoneArgs, output_format: OutputFormat) -> anyhow::Result<()> {
    let milestone_id = args.remove.ok_or_else(|| anyhow::anyhow!("Milestone ID is required"))?;
