use crate::models::{PetbathRow, ListItem};
...
pub fn handle_list(tag: Option<String>, format: OutputFormat) -> Result<()> {
    ...
    let entries: Vec<&crate::models::PetbathEntry> = storage::filter_by_tag(&store, tag.as_deref());
    ...
    let rows: Vec<PetbathRow> = entries.iter().map(|e| PetbathRow::from_entry(e)).collect();
