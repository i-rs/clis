...
pub fn handle_get(id: String, format: OutputFormat) -> Result<()> {
    ...
    print_header(&format!("Water: {}", display_id.green()));
    println!();

    let style = OwoStyle::new().bold();

    println!("{:16} {}", "Amount:".style(style), format!("{} ml", entry.amount_ml).cyan());
    ...
    println!("{:16} {}", "Time:".style(style), entry.drank_at.format("%Y-%m-%d %H:%M:%S").to_string().dimmed());
