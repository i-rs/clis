...
pub fn handle_get(id: String, format: OutputFormat) -> Result<()> {
    ...
    print_header(&format!("Calorie: {}", display_id.green()));
    println!();

    let style = OwoStyle::new().bold();

    println!("{:16} {}", "Food:".style(style), entry.food_name.cyan());
    println!("{:16} {}", "Calories:".style(style), format!("{} kcal", entry.calories).yellow());
    println!("{:16} {}", "Date:".style(style), entry.date.format("%Y-%m-%d").to_string().green());
