...
pub fn build_component_for(msg: &AgentMessage) -> Box<dyn MessageComponent> {
    match msg {
        ...
        AgentMessage::ToolResult { content, diff, step, total_steps, collapsed, .. } => {
            Box::new(tool_result::ToolResultCard::new(
                content, diff.as_deref(), *step, *total_steps, *collapsed,
            ))
        }
...
pub fn header_line(
    ...
    if let Some(m) = meta {
        spans.push(Span::styled(format!("  {}", m), Style::default().fg(c_muted())));
    }
    Line::from(spans)
}
...
fn split_rgb(c: Color) -> (u8, u8, u8) {
    if let Color::Rgb(r, g, b) = c { (r, g, b) } else { (128, 128, 128) }
}
...
impl BodyArea {
    #[allow(dead_code)]
    pub fn height(&self) -> u16 { self.bottom.saturating_sub(self.top) }
    pub fn contains(&self, y: u16) -> bool { y >= self.top && y < self.bottom }
}
...
pub fn render_block_chrome(
    ...
    if y_offset <= 1 {
        Paragraph::new(header)
            .style(bg_style)
            .render(Rect { y: area.y + 1 - y_offset, height: 1, ..area }, buf);
    }
