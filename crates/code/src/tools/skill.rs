    ...
    async fn call(&self, args: &Map<String, Value>) -> ToolResult {
        ...
        match action {
            "list" => {
                ...
                if skills.is_empty() {
                    return Ok("No skills installed. Create one with `i-rs-code skill create <name>`.".to_string());
                }
            ...
            "get" => {
                let name = args.get("name")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| crate::error::ToolError::invalid_args("Missing 'name' parameter for action='get'"))?;
                let store = SkillStore::new();
                match store.get(name) {
                    ...
                    None => Ok(format!("Skill '{}' not found. Use `skill` tool with action='list' to see available skills.", name)),
                }
            }
