use crate::models::{AllergyRow, ListItem};
...
pub fn handle_list(tag: Option<String>, format: OutputFormat) -> Result<()> {
    ...
    let entries: Vec<&crate::models::AllergyEntry> = storage::filter_by_tag(&store, tag.as_deref());
    ...
    let rows: Vec<AllergyRow> = entries.iter().map(|e| AllergyRow::from_entry(e)).collect();
