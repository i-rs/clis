...
pub fn handle_get(id: String, format: OutputFormat) -> Result<()> {
    ...
    if let Some(end) = entry.end_time {
        println!("{:16} {}", "End:".style(style), end.format("%Y-%m-%d %H:%M").to_string().red());
    }
    let status = if entry.end_time.is_some() { "COMPLETED" } else { "ACTIVE" };
    let status_color = if entry.end_time.is_some() { status.green().to_string() } else { status.yellow().to_string() };
