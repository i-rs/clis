...
pub fn handle_get(id: String, format: OutputFormat) -> Result<()> {
    ...
    print_header(&format!("Walkdog: {}", display_id.green()));
    println!();

    let style = OwoStyle::new().bold();

    println!("{:16} {}", "Dog:".style(style), entry.dog_name.cyan());
    println!("{:16} {}", "Duration:".style(style), format!("{} minutes", entry.duration_minutes).yellow());
    println!("{:16} {}", "Walked at:".style(style), entry.walked_at.format("%Y-%m-%d %H:%M").to_string().green());
