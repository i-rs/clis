...
pub fn get_data_path() -> PathBuf {
    if let Ok(config_dir) = std::env::var("CONFIG_DIR") {
        PathBuf::from(config_dir).join("i-rs").join("pigs.json")
    } else {
        let config_dir = dirs::config_dir().unwrap_or_else(|| PathBuf::from("."));
        config_dir.join("i-rs").join("pigs.json")
    }
}

pub fn load_store() -> Result<PigStore> {
...
pub fn save_store(store: &PigStore) -> Result<()> {
    let path = get_data_path();
