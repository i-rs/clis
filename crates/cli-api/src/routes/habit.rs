...
async fn list_habits(State(state): State<Arc<AppState>>) -> ApiResult<Json<serde_json::Value>> {
    let records = state
        .habit
        .read(|store| i_rs_habit::service::list_habits(store, None).map_err(ApiError::from))?;
...
async fn add_habit(
    ...
    let habit = state.habit.write(|store| {
        i_rs_habit::service::add_habit(store, name, description, frequency, tags, Vec::new())
...
async fn get_habit(
    ...
    let habit = state
        .habit
        .read(|store| i_rs_habit::service::get_habit(store, &name).map_err(ApiError::from))?;
    Ok(ok_json(habit))
}
...
async fn checkin_habit(
    ...
    let habit = state
        .habit
        .write(|store| i_rs_habit::service::checkin_habit(store, &name).map_err(ApiError::from))?;
    Ok(ok_json(habit))
}
