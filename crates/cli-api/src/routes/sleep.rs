...
async fn update_sleep(
    ...
    let entry = state.sleep.write(|store| -> Result<_, ApiError> {
        let entry = store
            .entries
            .get_mut(&id)
            .ok_or_else(|| ApiError::NotFound(format!("Sleep '{id}' not found")))?;
...
async fn list_sleeps(State(state): State<Arc<AppState>>) -> ApiResult<Json<serde_json::Value>> {
