...
pub fn router() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/{id}", delete(delete_cycling).patch(update_cycling))
}
...
async fn list_cyclings(State(state): State<Arc<AppState>>) -> ApiResult<Json<serde_json::Value>> {
...
async fn add_cycling(
    ...
    let entry = i_rs_cycling::models::CyclingRecord::new(
        date,
        distance_km,
        duration_minutes,
        elevation_gain,
