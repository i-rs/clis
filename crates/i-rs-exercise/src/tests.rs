...
mod tests {
    ...
    fn setup() {
        ...
        INIT.get_or_init(|| {
            let tmp = std::env::temp_dir()
                .join(format!("i-rs-exercise-test-{}", std::process::id()));
            // ensure dir exists
    ...
    #[test]
    fn test_help() {
        let _ = Cli::try_parse_from(["i-rs-exercise", "--help"]);
    }
    ...
    fn test_crud() {
        ...
        let cmd = Commands::Add {
            name: "test-i-rs-exercise-1".to_string(),
            exercise_type: "test-exercise_type".to_string(),
            duration_minutes: 1,
            calories: None,
            tag: vec![],
            notes: vec![],
        ...
        assert!(run(cmd, crate::presentation::OutputFormat::Table).is_ok());
        let cmd = Cli::try_parse_from(["i-rs-exercise", "list"]).unwrap().command;
        assert!(run(cmd, crate::presentation::OutputFormat::Table).is_ok());
        let cmd = Commands::Add {
            name: "test-i-rs-exercise-1b".to_string(),
            exercise_type: "test-exercise_type".to_string(),
            duration_minutes: 1,
            calories: None,
            tag: vec![],
