...
fn default_error() -> String { "bold red".into() }
fn default_success() -> String { "green".into() }
fn default_header() -> String { "bold cyan".into() }
fn default_warning() -> String { "yellow".into() }
fn default_table_border() -> String { "cyan".into() }
fn default_table_header() -> String { "bold cyan".into() }
fn default_table_row() -> String { "green".into() }
fn default_dimmed() -> String { "bright black".into() }
...
pub fn apply(text: &str, spec: &str) -> String {
    ...
    let color = parts.iter().find(|p| {
        !["bold", "dim", "dimmed", "italic", "underline", "bright"].contains(p)
    }).copied();

    macro_rules! c {
        ($e:expr) => {{
            let mut s = $e.to_string();
            if has_bold { s = s.bold().to_string(); }
            if has_dim { s = s.dimmed().to_string(); }
            s
        }};
    }
