use crate::models::{BedRow, ListItem};
...
pub fn handle_list(tag: Option<String>, format: OutputFormat) -> Result<()> {
    ...
    let entries: Vec<&crate::models::BedEntry> = storage::filter_by_tag(&store, tag.as_deref());
    ...
    let rows: Vec<BedRow> = entries.iter().map(|e| BedRow::from_entry(e)).collect();
