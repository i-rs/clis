...
pub trait McpServerConfigRepo: Send + Sync {
    ...
    async fn delete(&self, user_id: &str, agent_id: Option<&str>, name: &str)
    -> anyhow::Result<()>;
}
...
pub struct FileAgentConfigStore {
    claw_dir: PathBuf,
}
impl FileAgentConfigStore {
...
impl AgentConfigRepo for FileAgentConfigStore {
    ...
    async fn upsert(&self, row: &AgentConfigRow) -> anyhow::Result<()> {
        let path = self
            ...
            .join("agent_configs.json");
        let _ =
            std::fs::create_dir_all(path.parent().expect("agent config path should have parent"));
...
pub struct FileDashboardUserStore {
    claw_dir: PathBuf,
}
impl FileDashboardUserStore {
    pub fn new(claw_dir: PathBuf) -> Self {
        Self { claw_dir }
    }
}
...
impl DashboardUserRepo for FileDashboardUserStore {
    ...
    async fn upsert(&self, row: &DashboardUserRow) -> anyhow::Result<()> {
        ...
        let _ = std::fs::create_dir_all(
            path.parent()
                .expect("dashboard user config path should have parent"),
        );
        let mut rows = self.load_all().await?;
...
pub struct FileMcpServerConfigStore {
    claw_dir: PathBuf,
}
impl FileMcpServerConfigStore {
    pub fn new(claw_dir: PathBuf) -> Self {
        Self { claw_dir }
    }
}
...
impl McpServerConfigRepo for FileMcpServerConfigStore {
    async fn load_for(
        ...
        let all: Vec<McpServerConfigRow> =
            serde_json::from_str(&std::fs::read_to_string(&path)?).unwrap_or_default();
        Ok(all
            .into_iter()
            .filter(|r| r.user_id == user_id)
            .filter(|r| {
                r.agent_id.as_deref() == agent_id || (r.agent_id.is_none() && agent_id.is_none())
            })
            .collect())
    }
    async fn upsert(&self, row: &McpServerConfigRow) -> anyhow::Result<()> {
        ...
        rows.retain(|r| {
            !(r.user_id == row.user_id
                && r.agent_id.as_deref() == row.agent_id.as_deref()
                && r.name == row.name)
        });
        rows.push(row.clone());
        crate::utils::atomic_write(&path, &serde_json::to_string_pretty(&rows)?)?;
    ...
    async fn delete(
        ...
        rows.retain(|r| {
            !(r.user_id == user_id && r.agent_id.as_deref() == agent_id && r.name == name)
        });
        crate::utils::atomic_write(&path, &serde_json::to_string_pretty(&rows)?)?;
        Ok(())
    }
...
pub struct FileAppSettingsStore {
    claw_dir: PathBuf,
}
impl FileAppSettingsStore {
    pub fn new(claw_dir: PathBuf) -> Self {
        Self { claw_dir }
    }
}
...
impl AgentConfigRepo for EmptyAgentConfigStore {
    async fn load_all(&self, _user_id: &str) -> anyhow::Result<Vec<AgentConfigRow>> {
        Ok(vec![])
    }
    async fn upsert(&self, _row: &AgentConfigRow) -> anyhow::Result<()> {
        Ok(())
    }
    async fn delete(&self, _user_id: &str, _agent_id: &str) -> anyhow::Result<()> {
        Ok(())
    }
}
...
impl ProviderConfigRepo for EmptyProviderConfigStore {
    ...
    async fn upsert(&self, _row: &ProviderConfigRow) -> anyhow::Result<()> {
        Ok(())
    }
...
impl DashboardUserRepo for EmptyDashboardUserStore {
    ...
    async fn upsert(&self, _row: &DashboardUserRow) -> anyhow::Result<()> {
        Ok(())
    }
    async fn find_by_token_hash(&self, _hash: &str) -> anyhow::Result<Option<DashboardUserRow>> {
        Ok(None)
    }
...
impl McpServerConfigRepo for EmptyMcpServerConfigStore {
    ...
    async fn upsert(&self, _row: &McpServerConfigRow) -> anyhow::Result<()> {
        Ok(())
    }
    async fn delete(
        &self,
