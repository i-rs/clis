use crate::models::{FeedpetRow, ListItem};
...
pub fn handle_list(tag: Option<String>, format: OutputFormat) -> Result<()> {
    ...
    let entries: Vec<&crate::models::FeedpetEntry> = storage::filter_by_tag(&store, tag.as_deref());
    ...
    let rows: Vec<FeedpetRow> = entries.iter().map(|e| FeedpetRow::from_entry(e)).collect();
