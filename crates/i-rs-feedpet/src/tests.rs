...
mod tests {
    ...
    fn setup() {
        ...
        INIT.get_or_init(|| {
            let tmp = std::env::temp_dir()
                .join(format!("i-rs-feedpet-test-{}", std::process::id()));
            // ensure dir exists
    ...
    #[test]
    fn test_help() {
        let _ = Cli::try_parse_from(["i-rs-feedpet", "--help"]);
    }
    ...
    fn test_add_and_list() {
        ...
        let cmd = Commands::Add {
            pet_name: "test-i-rs-feedpet-1".to_string(),
            food_type: "test-food_type".to_string(),
            amount: "test-amount".to_string(),
            tag: vec![],
            remark: vec![],
        };
        assert!(run(cmd, crate::presentation::OutputFormat::Table).is_ok());
        let cmd = Cli::try_parse_from(["i-rs-feedpet", "list"]).unwrap().command;
        assert!(run(cmd, crate::presentation::OutputFormat::Table).is_ok());
    }
