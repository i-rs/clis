...
pub fn handle_get(id: String, format: OutputFormat) -> Result<()> {
    ...
    print_header(&format!("Sheet: {}", display_id.green()));
    println!();

    let style = OwoStyle::new().bold();

    println!("{:16} {}", "Type:".style(style), entry.sheet_type.cyan());
    println!("{:16} {}", "Changed:".style(style), entry.changed_at.format("%Y-%m-%d %H:%M").to_string().yellow());
