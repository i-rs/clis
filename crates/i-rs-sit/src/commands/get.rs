...
pub fn handle_get(id: String, format: OutputFormat) -> Result<()> {
    ...
    println!("{:16} {}", "Duration:".style(style), format!("{} minutes", entry.duration_minutes).cyan());
    println!("{:16} {}", "Started:".style(style), entry.started_at.format("%Y-%m-%d %H:%M").to_string().yellow());
    println!("{:16} {}", "Ended:".style(style), entry.ended_at.format("%Y-%m-%d %H:%M").to_string().yellow());
