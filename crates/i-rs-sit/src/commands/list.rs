use crate::models::{SitRow, ListItem};
...
pub fn handle_list(tag: Option<String>, format: OutputFormat) -> Result<()> {
    ...
    let entries: Vec<&crate::models::SitEntry> = storage::filter_by_tag(&store, tag.as_deref());
    ...
    let rows: Vec<SitRow> = entries.iter().map(|e| SitRow::from_entry(e)).collect();
