use crate::models::{AcRow, ListItem};
...
pub fn handle_list(tag: Option<String>, format: OutputFormat) -> Result<()> {
    ...
    let entries: Vec<&crate::models::AcEntry> = storage::filter_by_tag(&store, tag.as_deref());
    ...
    let rows: Vec<AcRow> = entries.iter().map(|e| AcRow::from_entry(e)).collect();
