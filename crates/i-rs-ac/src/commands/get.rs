...
pub fn handle_get(id: String, format: OutputFormat) -> Result<()> {
    ...
    print_header(&format!("AC: {}", display_id.green()));
    println!();

    let style = OwoStyle::new().bold();

    println!("{:16} {}", "Location:".style(style), entry.location.cyan());
    println!("{:16} {}", "Cleaned at:".style(style), entry.cleaned_at.format("%Y-%m-%d %H:%M").to_string().yellow());
