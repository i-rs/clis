...
impl SqliteBackend {
    ...
    async fn migrate(&self) -> anyhow::Result<()> {
        ...
        sqlx::query("CREATE TABLE IF NOT EXISTS memory (agent_id TEXT PRIMARY KEY, data TEXT NOT NULL)").execute(&self.pool).await?;
...
define_sql_stores!(
    sqlx::SqlitePool, SqliteBackend,
    SqliteSessionStore, SqliteMessageStore, SqliteApiCacheStore, SqlitePlanStepsStore,
    SqliteMemoryStore, SqliteStatsStore, SqliteSkillStore, SqliteToolCacheStore,
...
mod tests {
    ...
    async fn test_session_save_load() {
        ...
        s.sessions.save_all(&[crate::session::SessionMeta { id: "s1".into(), title: "Hi".into(), agent_id: "d".into(), state: crate::session::SessionState::Active, created_at: 1, updated_at: 2, message_count: 0 }]).await.unwrap();
    ...
    async fn test_message_append_load() {
        let s = test_storage().await;
        s.sessions.save_all(&[crate::session::SessionMeta { id: "s1".into(), title: "T".into(), agent_id: "d".into(), state: crate::session::SessionState::Active, created_at: 1, updated_at: 2, message_count: 0 }]).await.unwrap();
        s.messages.append("s1", &serde_json::json!({"type":"user","text":"hi"})).await.unwrap();
