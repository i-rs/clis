...
pub fn handle_get(id: String, format: OutputFormat) -> Result<()> {
    ...
    print_header(&format!("Dose: {}", display_id.green()));
    println!();

    let style = OwoStyle::new().bold();

    println!("{:16} {}", "Medicine:".style(style), entry.medicine_name.cyan());
    println!("{:16} {} {}", "Dosage:".style(style), entry.dosage.yellow(), entry.unit);
    ...
    println!("{:16} {}", "Time:".style(style), entry.taken_at.format("%Y-%m-%d %H:%M:%S").to_string().dimmed());
