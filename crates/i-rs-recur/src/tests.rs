...
mod tests {
    ...
    fn setup() {
        ...
        INIT.get_or_init(|| {
            let tmp = std::env::temp_dir()
                .join(format!("i-rs-recur-test-{}", std::process::id()));
            // ensure dir exists
    ...
    #[test]
    fn test_help() {
        let _ = Cli::try_parse_from(["i-rs-recur", "--help"]);
    }
    ...
    fn test_crud() {
        ...
        let cmd = Commands::Add {
            name: "test-i-rs-recur-1".to_string(),
            amount: 1.0,
            currency: "test-currency".to_string(),
            frequency: "test-frequency".to_string(),
            start_date: "2024-01-15".to_string(),
            tag: vec![],
            remark: vec![],
        };
        assert!(run(cmd, crate::presentation::OutputFormat::Table).is_ok());
        let cmd = Cli::try_parse_from(["i-rs-recur", "list"]).unwrap().command;
        assert!(run(cmd, crate::presentation::OutputFormat::Table).is_ok());
        let cmd = Commands::Add {
            name: "test-i-rs-recur-1b".to_string(),
            amount: 1.0,
            currency: "test-currency".to_string(),
            frequency: "test-frequency".to_string(),
            start_date: "2024-01-15".to_string(),
