use crate::models::{AquaRow, ListItem};
...
pub fn handle_list(tag: Option<String>, format: OutputFormat) -> Result<()> {
    ...
    let entries: Vec<&crate::models::AquaEntry> = storage::filter_by_tag(&store, tag.as_deref());
    ...
    let rows: Vec<AquaRow> = entries.iter().map(|e| AquaRow::from_entry(e)).collect();
